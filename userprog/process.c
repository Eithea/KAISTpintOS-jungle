...
lazy_load_segment (struct page *page, void *aux) {
	...
	/// 3-2
	// aux로부터 로드 정보를 뽑아 커널 세팅
	// read만큼 read, zero만큼 memset 0
	struct file *file = ((struct lazy_load_info *)aux)->file;
	off_t ofs = ((struct lazy_load_info *)aux)->ofs;
	size_t page_read_bytes = ((struct lazy_load_info *)aux)->page_read_bytes;
    size_t page_zero_bytes = PGSIZE - page_read_bytes;
...
setup_stack (struct intr_frame *if_) {
	...
	/* TODO: Your code goes here */
	/// 3-2
	// 페이지 할당 후 유저 스택에 해당하는 페이지를 가져와 물리/가상 매핑
	// 성공시 rsp를 유저 스택으로 세팅, curr의 스택 포인터 갱신
